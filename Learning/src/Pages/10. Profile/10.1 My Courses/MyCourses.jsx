...
export default function MyCourses() {
    ...
    if (loggedInUser) {
        const user = existingUserData.find((user) => user.email === loggedInUser.email);
  ...
  const removeCourse = (courseId) => {
    ...
    if (loggedInUser) {
      const userIndex = existingUserData.findIndex((user) => user.email === loggedInUser.email);

      if (userIndex !== -1) {
        const user = existingUserData[userIndex];
        user.myCourse = user.myCourse.filter((course) => course.id !== courseId);
      ...
      <div className='border rounded-xl pb-2 w-full'>
        {userCourses.length > 0 ? (
          userCourses.map((course, index) => (
            <div key={course.id} className="flex my-4 mx-20 border p-4 justify-between">
              <div className='flex'>
                <div className='mr-4 text-2xl font-bold flex items-center justify-center p-2'>{index + 1}</div>
                <img src={course.imageCourse} alt={course.title} className="w-30 h-auto mr-4" />
